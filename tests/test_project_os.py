...
class ClosedEpicDependencies(unittest.TestCase):
    ...
    def test_the_closing_set_is_exactly_these_two(self) -> None:
        """An exact assertion, not a few named exclusions.

        A behavioural test over today's statuses cannot object to a third member being added, and a
        third member would satisfy every dependency in the backlog at once. Widening this is a
        policy change: docs/development/WORK_ITEM_MODEL.md has to change alongside the assertion,
        which is the point of making it fail (E29-S07).
        """
