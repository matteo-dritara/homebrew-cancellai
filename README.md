...
- **Never touched:** Claude Code's auto-memory, `settings.json`, `keybindings.json`, plugins/skills/agents/commands; Codex's `auth.json`, `config.toml`, plugins/skills/rules/memories. These names are enforced twice - when the plan is built and again immediately before any deletion - and the check is applied to the path as written as well as after resolution, so a protected entry that is a symlink keeps its protection.
...
- **The root must prove it is a provider root, and you must mean it.** Your real `~/.codex` and `~/.claude` are authoritative by definition. A relocated root is only ever deleted from when it contains content-validated provider data *and* you pass `--allow-custom-root`; a matching filename alone proves nothing. Inspection is never gated. See [ADR-0012](docs/adrs/0012-custom-provider-roots-require-structure-and-intent.md).
...
cancellai clean --allow-custom-root        # required for a relocated CODEX_HOME/CLAUDE_CONFIG_DIR
...
Exit code 4 is what you get when a Codex or Claude process is running or cannot be observed, when a configured root fails its fingerprint, or when part of the scan was unreadable. cancellAI skips that tool rather than guessing, and a safety boundary that fires mid-run is reported through this exit code and the JSON result rather than as a crash.
...
The architecture review of 2026-08-27 identified seven P0 trust-floor defects in the Python implementation, scheduled in roadmap epic `E00`. They went through a first remediation round and an independent review that rejected six of the seven; every defect that review found has been repaired and the work is awaiting a second review. Nothing is closed until a reviewer issues a verdict. See [the baseline code review](docs/audits/2026-08-27-CODE_REVIEW.md), [the verifier review](project/evidence/E00-VERIFIER-REVIEW.md) and [the as-is architecture](docs/architecture/AS_IS.md).
...
Read [AGENTS.md](AGENTS.md) before changing the project, whether you are a human or a coding agent. See [.github/CONTRIBUTING.md](.github/CONTRIBUTING.md) and [.github/SECURITY.md](.github/SECURITY.md).
