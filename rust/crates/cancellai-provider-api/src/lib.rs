...
//! [`CapabilityOutcome`]. `protection`/`root_probe` (E05-S03) are tool-agnostic utilities
//! every adapter needs (protected-name comparison, provider-root marker probes) ported from
//! `cancellai.py`, kept here rather than duplicated per adapter crate since neither carries
//! any provider-specific knowledge of its own. The manifest model (declarative
//! root/pattern/category knowledge, PROVIDER_MODEL.md "Manifest-only" integration level) does
//! not exist yet and is deferred to a later E05 story.
