...
## Regression detectors and correctness oracles are not the same evidence

**A fixture recorded from an implementation detects change. It cannot detect wrongness.** The
`NORMATIVE` corpus is generated by `scripts/characterize.py generate` from `cancellai.py`'s observed
behaviour, so "the implementation matches its characterization" proves the implementation matches
itself. Where the recorded behaviour was wrong, the defect is in the oracle - an *acceptability
fallacy* in Greenwell's taxonomy of fallacies in system safety arguments.

`KNOWN_DEFECT` mitigates half of it: a recorded behaviour can be marked "must not be reproduced",
and a reclassification updates the record and its rationale in the same change. It cannot help with
the defects nobody noticed, which are recorded as `NORMATIVE` and become requirements.

So, explicitly:

- **Regression detectors** - the characterization corpus, the golden schema documents, the
  differential parity corpus. They answer "did this change?" and nothing more.
- **Correctness oracles** - `scripts/safety_oracle.py`, whose predicates for protected-name
  enforcement, root capability and the retention rule are written from
  [`SAFETY_INVARIANTS.md`](../security/SAFETY_INVARIANTS.md) and the README rather than from either
  implementation. A disagreement with a predicate is a failure **even when both engines agree with
  each other**: agreement between two implementations of the same misunderstanding is not evidence.

The oracle covers three behaviours, chosen because being wrong about them cannot be undone, and it
currently checks the Python reference only (E25-S07). Everywhere else the evidence is a regression
detector, and reading it as more than that is the mistake this section exists to prevent.
