...
- If a Windows target is a directory, a reparse point, or of any unconfirmed kind, then the executor shall refuse it as it does today; a regular file with further hard links is deleted as one name, as on Unix and in the reference, because unlinking a name never removes data another link still reaches (amended 2026-09-23 by the owner after E06 review round 3 held the original wording to a refusal this criterion should not have demanded).
...
- A cross-platform CLI test shows a hard-linked stale session deleted by name while the other link keeps its bytes.
