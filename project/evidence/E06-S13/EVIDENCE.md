...
## Round-4 repair (Codex, `project/evidence/E06-VERIFIER-REVIEW-ROUND4.md`)

| Finding | Repair | Evidence |
| --- | --- | --- |
| A second link added after the open-time check was detected only after the planned name had been removed (Unix); the Windows delete handle had no link check of its own | The link count is now checked where the deletion happens: on Unix in the same `fstatat` that confirms identity immediately before `unlinkat`, on Windows on the very handle the delete disposition is set on (`SealError::MultipleLinks`). The open-time and post-unlink checks stay. A link added between that final read and the syscall is the owner-accepted leaf-name race window. | `a_hard_link_added_after_the_open_time_check_is_refused_before_any_name_is_removed` (Unix) and `windows_a_hard_link_added_after_the_open_time_check_is_refused_before_deletion` (Windows CI): refused, both names keep their bytes. Removing the sealedfs check fails the Unix test. |
