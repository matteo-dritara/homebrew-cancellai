...
| --- | --- | --- |
| AC2 as written demanded refusing a hard-linked Windows file, and the code deletes it | The owner amended AC2 (2026-09-23): unlinking a name never removes data another link reaches, which is also the Unix and reference behaviour, so a hard-linked file is deleted as one name. The new test then exposed a defect on **Unix** (E21-S07's primitive): the post-unlink check demanded a link count of 0 on the held descriptor, so a hard-linked file was unlinked and reported as a failed deletion of "a different object". It now requires exactly one link fewer than before, which still fails if the unlink removed a different object (the held object's count would not change). | `a_hard_linked_session_is_deleted_as_one_name_and_the_other_link_survives` (every platform, stable channel): the provider's name is removed, the outside link keeps its bytes, the run reports success |
