...
| AC3 | `revalidated` is a dated field the refusal message tells the reader to set; passing once does not set it. `tests/test_skill_content.py::WaiversAreBoundToTheScannerThatProducedThem::test_the_committed_file_records_when_it_was_revalidated` | PASS |
...
- `revalidated` is a date nobody checks against the scanner version it belongs to, so a wrong date is expressible.
