...
| AC2 - more than MAX_RETAINED_REASONS companion failures: bounded, exact, Partial, withheld | `walk_companion_payload` takes the scope's `&mut ReasonLog` and records each failure as it happens; the intermediate `Vec` is gone. `more_companion_failures_than_retained_stay_bounded_exact_and_partial` (70 failures): `unobserved_count` = 70, 64 retained, Partial, companion attributed as degraded. CLI level: NORMATIVE fixture `claude-many-companion-failures` withholds the tool on both engines. | PASS |
| AC3 - both cases match the reference in default and custom root origins | `python3 scripts/rust_python_parity.py check`: 15 NORMATIVE fixtures match in both root-origin scenarios. Against the unrepaired Codex adapter the same gate fails on `codex-unreadable-rollout` in both scenarios (Rust candidates = all three rollouts, `scan_complete` true), so the fixture detects the defect. | PASS |
...
python3 scripts/characterize.py generate && check                        -> 15 fixtures
python3 scripts/check_fixtures.py check                                  -> OK
python3 scripts/rust_python_parity.py check                              -> 15 NORMATIVE, both origins
