...
| AC | Evidence | Result |
| --- | --- | --- |
| AC1 | `scripts/check_skill_content.py check` runs SkillSpector over `.claude/skills` and reads the per-skill `issues`; the verdict comes from that scan, not from the manifest's `trust` field. `tests/test_skill_content.py::SeverityLadder` | PASS |
